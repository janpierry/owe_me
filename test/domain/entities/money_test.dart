...
void main() {
  ...
  group(
    ...
    () {
      test(
        'Should string value of a money be shown in Real currency (using R\$ and comma instead of dot)',
        () {
          const value = Money(cents: 111);
          expect(value.toString(), 'R\$1,11');
        },
      );
      test(
        'Should string value of a money be written preciselly even if there is not a single real',
        () {
          const value = Money(cents: 1);
          expect(value.toString(), 'R\$0,01');
        },
      );
    },
  );
